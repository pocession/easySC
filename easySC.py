...
class SCAnalysis(paths):
    ...
    def load_data(self):
        ...
        print(
            f"Your data set contains {self.data.n_obs} cells and {self.data.n_vars} genes."
        )
    ...
    def filter_data(self):
        ...
        print(
            f"After filtering, your data set contains {self.data.n_obs} cells and {self.data.n_vars} genes."
        )
    ...
    def qc_data(self):
        ...
        self.data.var["mt"] = self.data.var_names.str.startswith(
            "MT-"
        )  # annotate the group of mitochondrial genes as 'mt'
        sc.pp.calculate_qc_metrics(
            self.data, qc_vars=["mt"], percent_top=None, log1p=False, inplace=True
        )
        sc.pl.violin(
            self.data,
        ...
        sc.pl.scatter(
            self.data, x="total_counts", y="pct_counts_mt", show=False, save="_mtQC.png"
        )
        sc.pl.scatter(
            self.data,
            x="total_counts",
